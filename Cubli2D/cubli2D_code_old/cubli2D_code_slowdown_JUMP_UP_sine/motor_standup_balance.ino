...
void stand_up()
{
  ...
  {  
    ...
    while (millis() - standup_timer <= 3000)
    {
...
//  {
//    sine_input();
//    if (sensor == 2)  curr = (((k1 * spw + k2 * angle_sine + k3 * spf)) / kt);  // IMU controller with sine input
//  }
  if (curr >= CURRENT_MAX)  curr = CURRENT_MAX;  // if above max current set equal to max
  if (curr <= -CURRENT_MAX)  curr = -CURRENT_MAX; // if below min current set equal to min
  duty = (int)interpolate(curr, -CURRENT_MAX, CURRENT_MAX, freq_max, freq_min); // map the current from max to min
...
//Function for landing the Cubli using braking
void touchdown_brake()
{
  sensor = 1; //use potentiometer for angle measurements 
...
//Function for landing the Cubli using slowdown
...
//Function for sine reference input for the angle. 
void sine_input()
{
  if (start_sine == true) //statement in order to always start the sine from 0.
  {
    timer_sine = millis();
    angle_sine = ang_err-A*sin(B*(millis()-timer_sine)/1000);
    start_sine = false;
  }
  if (start_sine == false)
  {
