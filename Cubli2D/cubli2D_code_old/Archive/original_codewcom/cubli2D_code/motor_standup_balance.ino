...
void standup() {
  ...
  if (abs(angle_pot()) > recovery) {  // if we are down
    ...
    brake.write(brk - 5); //brake motor hard to get up propper
    delay(75);            //wait before diabling the brake
...
void updateMotor() {
  ...
  //txdata.packet.spf = spf; // set txpacket spf
  //txdata.packet.ang_err = ang_err; // set txpacket ang_err
  //txdata.packet.spw = spw; // set txpacket spw
