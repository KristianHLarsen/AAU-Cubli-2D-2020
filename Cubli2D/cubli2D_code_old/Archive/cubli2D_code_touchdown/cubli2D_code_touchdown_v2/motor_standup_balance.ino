...
float speed_frame() {
  ...
  if (sensor == 2) angle_speed2 = (GyZ * 0.0174532925);  //Gyroscope is speed of frame
  return angle_speed;
}

float speed_frame2() {
  if (sensor == 0) angle_speed2 = (GyZ * 0.0174532925);  //Gyroscope is speed of frame
  return angle_speed2;
}
...
//  Serial.print(spw);
  ...
  if (ang_err > recovery - 21*3.14/180 && !touchdown_brake)
  {
      brake.write(brk - 5); 
      touchdown_measurement_timer = millis();
      Serial.println("in loop");
      while (millis() - touchdown_measurement_timer <= 50) // printing data to serial port while braking
      {
  ...
  else if (ang_err < -recovery + 24*3.14/180 && !touchdown_brake)
  {
      brake.write(brk - 5); 
      touchdown_measurement_timer = millis();
      Serial.println("in loop");
  ...
  if (abs(ang_err) > recovery || millis() - touchdown_timer> 800)
    {
      FPGA.analogWrite(PWM_PIN, map(50, 0, 100, pow(2, bits), 0)); //stop motor
      digitalWrite(enable, LOW); // disable driver
    }
  else if (abs(ang_err) > TD_ANGLE_START)
  {
    touchdown_curr = TD_GAIN * sin(ang_err);
